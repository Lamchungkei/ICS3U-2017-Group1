...
class Level1Scene(Scene):
    ...
    def update(self):
        ...
        if self.jump_button_down == True:
           # move up squareman
           if self.squareman.position.y < self.size_of_screen_y:
              squaremanMoveUp = Action.move_by(0.0, self.squareman_move_speed, 0.1)
              self.squareman.run_action(squaremanMoveUp)
            
        if self.jump_button_down == False:
           # move down squareman
           if self.squareman.position.y > self.screen_center_y - 25:
              squaremanMoveDown = Action.move_by(0.0, -1*self.squareman_move_speed, 0.1)
              self.squareman.run_action(squaremanMoveDown)
